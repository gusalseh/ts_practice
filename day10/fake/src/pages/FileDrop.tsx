...
export default function FileDrop() {
  const [imageUrls, setImageUrls] = useState<string[]>([]); // Base64 인코딩 문자열로 변환된 이미지 데이터
  ...
  // makeImageUrls 함수는 사용자가 선택한 이미지 파일들을 imageUrls 상태에 계속 추가하는 역활을 한다.
  ...
  const images = useMemo(() => 
    imageUrls.map((url, index) => (
      <Div key={index} src={url} 
        className="m-2 bg-trasparent bg-center bg-no-repeat bg-contain" 
